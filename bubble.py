...
No seu caso vc implementou uma melhoria em seu código. Vc sempre elimina o último elemento de sua nova lista. Então ele vira 
(n-1)+(n-2)+(n-3)...1 = (n-1+1)*(n-1)/2=(n**2-n)/2. Veja que ele ainda é O(n**2) nesse caso. 
O pior caso acontece quando o elemento mínimo é o ultimo. Nos outros casos vc pode atingir o break antes e a fórmula não vai funcionar.
Veja que no seu caso n=10 o 0 é minimo e ultimo. (10**2-10)/2 dá exatamente os 45 que vc encontrou.
